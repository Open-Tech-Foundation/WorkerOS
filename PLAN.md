...
| **M5 — Ecosystem** | 5–6 | npm install + Vite dev preview — 🚧 `npm` registry install + `node` CommonJS `require` done; **Node compat: sync `fs`, core builtins (`fs`/`path`/`os`/`url`/`module`), and ESM resolution — `node:` builtins + `node_modules` (`exports`/subpaths) — done** (Phase 5·A–D); pending: bin-linking+PATH (E), CJS-in-ESM interop, preview + lockfiles |
