...
- **E · npm bin-linking + PATH** ✅ — **done**. `npm install` writes a generated launcher to `node_modules/.bin/<name>` for each package `bin` (string or map; the VFS has no symlinks, so the launcher re-execs `node <target>` via `sys.exec`, forwarding argv + exit code), and the kernel's command resolution (`resolver.rs` `node_bin_dirs`) searches `node_modules/.bin` walking up from `cwd` **ahead of** `/bin:/sbin` — so an installed package's command runs as a bare name, the standard Unix/npm model, with **no OS-specific shim in core** (the kernel just searches a path; `npm` writes a userland file). Honest limit (INV-5): `sys.exec` does not forward stdin to the bin yet.
