...
The goal is that **real npm packages and their bins run on the host JS engine** via the guest node layer (INV-1) — *not* by hosting a foreign runtime (see the out-of-scope note). The concrete proof driving the scope is **esbuild-wasm** — its own `bin/esbuild` needs `module`/`path`/**synchronous `fs`** — with the ESM path (workstreams C/D) proven by an ESM-only package that imports `node:` builtins and subpath-exported deps. Workstreams, roughly in dependency order:
...
- **C · `node:` builtin resolution** ✅ — **done, both module systems, all in userland.** CJS: `require('fs'/'node:fs'/'fs/promises'/'path'/…)` resolve to the builtin registry (`makeBuiltins`). ESM: `/bin/node` resolves its own graph (`esm-graph.js`) and marks a `node:` import (or a bare core name) as a **builtin edge**, then synthesizes a re-export module (`export default m` + a named export per key) wired to the guest runtime — so `import fs from 'node:fs'` and `import { readFileSync } from 'fs'` both work. The guest runtime library is installed at `/lib/workeros-node/`. (The kernel has no `node:` concept — INV-1.)
- **D · ESM `import` of installed packages** ✅ — **done** (subset), resolved **in userland, not the kernel** (INV-1 — `node_modules`/`exports`/`node:` is Node policy, not OS internals). Now that `/bin/node` has synchronous `fs` it builds its own ES module graph: `src/node/resolve.js` walks `node_modules` (nearest `node_modules/<name>` climbing to `/`) and applies package.json `exports`(".")/`module`/`main` with ESM condition selection (`import`/`node`/`default`), **subpath exports** (incl. `@scope` and `./*` wildcards), and `.js`/`.mjs`/`.cjs`/`.json` + `index` fallbacks; `src/node/esm-graph.js` scans imports (tokenized) and walks the graph. The kernel resolver does only **generic relative** resolution (used to spawn OS programs); a bare/`node:` specifier there is an honest `Unsupported`. **CJS-in-an-ESM-graph interop** is also done: a CommonJS dep reached via `import` is stood up by a synthetic ES module (`export default module.exports` + named-export interop) backed by the synchronous CJS loader (Phase 5·A/B), which resolves the dep's own `require` subtree on demand. Still ⏳: `browser` field and full `exports` pattern/condition fidelity.
- **E · npm bin-linking + PATH** ✅ — **done**, kept clean of the kernel. `npm install` writes a generated launcher to `node_modules/.bin/<name>` for each package `bin` (string or map; the VFS has no symlinks, so the launcher re-execs `node <target>` via `sys.exec`, forwarding argv + exit code). The **kernel resolves a bare command against `$PATH` from the environment** (falling back to `/bin:/sbin`) and knows nothing about `node_modules` (INV-1); the **shell** (`shell-exec.js`) prepends the `node_modules/.bin` chain (cwd + ancestors) to `PATH` before spawning, and `npm run` inherits it via `sys.exec` — exactly npm's model (edit the env, drop a launcher file; never teach the OS about `node_modules`). A launched bin inherits the controlling terminal, so `process.stdin` reaches it (see Phase 5·G).
- **F · global install (`-g`) + `npm create` / `npm init <x>`** ✅ — **done**, same userland discipline. `npm install -g` installs into the persistent global store `/.node_modules` (launchers in `/.node_modules/.bin`); the OS ships a default `/etc/profile` (an OS-owned ephemeral tree, reshipped each boot; user overrides in the persistent `~/.profile`) that the login shell sources at startup to put that dir on `$PATH` — so global bins run as bare commands anywhere while the **kernel resolver stays `/bin:/sbin`** (INV-1). `npm create <x>` / `npm init <x>` fetch the scope/version-aware `create-*` initializer into an ephemeral `/tmp` prefix and run its bin under `/bin/node` in the current cwd (npx semantics). Same install path for local and global via a `{ nmRoot, binDir }` context. The initializer inherits the controlling terminal, so interactive scaffolders can prompt (Phase 5·G). Global bin resolution is covered by `workeros-web` `tools/global-bin.test.js`.
- **G · interactive `process.stdin`** ✅ — **done**. `process.stdin` now reads fd 0: `node:tty`'s `ReadStream` became a real readable (a pump loop drains the fd — blocking in the kernel until keystrokes/EOF — driving flowing/paused/`setEncoding`/`pipe`/async-iteration, plus `isTTY`/`setRawMode` and event-loop ref/unref). Before this the stream never touched the fd, so `stdin.on('data')` was silent and every interactive program hung; the kernel side (fd 0 bound to the controlling TTY, cooked/raw line discipline, blocking reads) was already correct — the gap was purely the guest runtime never pumping. This makes readline/prompt-driven CLIs and `npm create` scaffolders work end-to-end, in cooked and raw/keypress modes. `readline.emitKeypressEvents` is also real now — it decodes the stream into Node `keypress` events (arrows/CSI+SS3, Home/End/Insert/Delete/PageUp·Down, F1–F12, xterm modifiers, shift-Tab, Alt/meta), so arrow-key prompt libraries (prompts/enquirer/inquirer) drive `process.stdin` in raw mode. Covered by `workeros-web` `tools/stdin-stream.test.js` (flowing, exec'd-child, raw, and an arrow-key menu) plus `workeros-programs` `tools/readline-keypress.test.js`.

**Proof targets (add opt-in browser tests when green):** `npm install esbuild-wasm && esbuild src/main.ts --bundle` runs the package's own bin under `/bin/node` (A+B+C+E); an ESM-only package installed into `node_modules` and imported from a script exercises the ESM resolution path (C+D+B). A spike already proved esbuild-wasm loads + bundles from the VFS, but via a bespoke `/bin/esbuild` driver that was **backed out** to keep core clean (Phase 4 note).
...
- **Temporal caps (kernel-worker watchdog, host-side — the only agent with a clock + `terminate()`) — ✅ implemented** (`watchdogTick`, values from `limits::WATCHDOG`, overridable per boot via `boot({ watchdog })`, `0` disables):
  - CPU-time budget (**30s of *continuous unresponsiveness*** — liveness = syscalls, PING/PONG, or parked in a kernel-serviced blocking call, so servers/blocked readers are never touched; ADR-020 deviation note); on breach, SIGTERM → grace → `worker.terminate()`, exit **152**, kernel-recorded reason `"CPU time"` (the `Ctrl-C` two-phase, generalized).
  - Memory high-water ceiling (**512 MiB**) via per-worker `performance.measureUserAgentSpecificMemory()` self-sampling where the browser exposes it to workers → `terminate()`, exit `137`, reason `"out of memory"`. **Soft/sampled, documented as such** (INV-5).
  - Proven in `tools/watchdog.test.js`: a `for(;;)` spin is reaped with 152 + `Killed (CPU time)`; an idle-but-responsive process and a stdin-blocked reader outlive the budget untouched; the OOM path is feature-gated.
- **Fault containment (all stop-paths reap through one seam) — 🚧 mostly:** ordinary exit + cooperative kill close IO + restore TTY ✅; guest crash (`worker.onerror`) reaps + unblocks downstream pipes + restores TTY like an exit ✅; **kill reason** recorded on the process record (`Kernel::mark_killed` → `ProcInfo.kill_reason`, plus a shell-visible `Killed (out of memory)` / `Killed (CPU time)` on stderr) ✅; a departed pipe reader breaks the writer with `EPIPE`/SIGPIPE instead of wedging it (ADR-023) ✅. Still ⏳: bounded `Atomics.wait` on the SAB path (a *kernel-worker* crash currently leaves a sync-parked guest waiting until the instance is torn down — the client sees the crash as fatal and reboots, so nothing observable wedges, but the wait itself is unbounded).
...
**Exit criteria:** a fork-bomb (`while true; do spawn & done`-shaped) hits the process cap with `EAGAIN` instead of killing the tab **✅**; filling the VFS past quota returns `ENOSPC` **✅**; a process exhausting its fd budget gets `EMFILE` **✅**; a synchronous infinite-loop process is reaped by the time watchdog with a `Killed (CPU time)` reason and does not freeze the kernel or siblings **✅** (`tools/watchdog.test.js`); a runaway allocator is `terminate()`d at the memory ceiling and reaps as `137` **✅ where the browser exposes worker memory measurement** (the test feature-gates; the kill path is shared with the proven CPU-time path); a crashing guest reaps deterministically and its downstream pipe reader sees EOF **✅** (`worker.onerror` → `handleExit`, ADR-023's pump). Accounting caps proven in native `cargo test` **✅** (`process_count_cap_refuses_forkbomb`, `open_fd_cap_returns_emfile_and_frees_on_close`, `byte_quota_*`/`inode_quota_*`).
...
| **M5 — Ecosystem** | 5–6 | npm install + Vite dev preview — 🚧 `npm` registry install + `node` CommonJS `require` done; **Node compat: sync `fs`, core builtins (`fs`/`path`/`os`/`url`/`module`), ESM resolution (`node:` builtins + `node_modules` `exports`/subpaths), CJS-in-ESM interop, and npm bin-linking+PATH, global `-g` install + `npm create` — done** (Phase 5·A–F); pending: preview + lockfiles |
