...
- **C · `node:` builtin resolution** ✅ — **done, both module systems**. CJS: `require('fs'/'node:fs'/'fs/promises'/'path'/…)` resolve to the builtin registry (`makeBuiltins`). ESM: the kernel resolver marks a `node:` import (or a bare core name in `NODE_BUILTINS`) as a **builtin edge** (`ImportEdge.builtin`), and `/bin/node`'s ESM stitch synthesizes a re-export module (`export default m` + a named export per key) wired to the guest runtime — so `import fs from 'node:fs'` and `import { readFileSync } from 'fs'` both work. The guest runtime library is installed at `/lib/workeros-node/`.
- **D · ESM `import` of installed packages** ✅ — **done** (subset). The kernel resolver (`resolver.rs`) walks `node_modules` (nearest `node_modules/<name>` climbing to `/`) and applies package.json `exports`(".")/`module`/`main` with ESM condition selection (`import`/`node`/`default`), **subpath exports** (incl. `@scope` and `./*` wildcards), and `.js`/`.mjs`/`.cjs`/`.json` + `index` fallbacks — resolved to real VFS files that join the graph, so ESM-only packages resolve. A dependency-free `json` parser reads the manifests (kernel stays `serde`-free). **CJS-in-an-ESM-graph interop** is also done: a CommonJS dep reached via `import` is stood up by a synthetic ES module (`export default module.exports` + named-export interop) backed by the synchronous CJS loader (Phase 5·A/B), which resolves the dep's own `require` subtree on demand. Still ⏳: `browser` field and full `exports` pattern/condition fidelity.
