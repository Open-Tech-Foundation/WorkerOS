...
- **G · interactive `process.stdin`** ✅ — **done**. `process.stdin` now reads fd 0: `node:tty`'s `ReadStream` became a real readable (a pump loop drains the fd — blocking in the kernel until keystrokes/EOF — driving flowing/paused/`setEncoding`/`pipe`/async-iteration, plus `isTTY`/`setRawMode` and event-loop ref/unref). Before this the stream never touched the fd, so `stdin.on('data')` was silent and every interactive program hung; the kernel side (fd 0 bound to the controlling TTY, cooked/raw line discipline, blocking reads) was already correct — the gap was purely the guest runtime never pumping. This makes readline/prompt-driven CLIs and `npm create` scaffolders work end-to-end, in cooked and raw/keypress modes. Covered by `workeros-web` `tools/stdin-stream.test.js`. Follow-up: `readline.emitKeypressEvents` (parse `'data'` into `keypress` events) so prompt libs that select on arrow keys via that helper light up — the byte stream it needs now exists.
