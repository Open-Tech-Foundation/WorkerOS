...
**Choice:** Persistence is a **projection of an authoritative in-memory tree**, not a backend swap, and durability is a property of the **path**, not a per-inode/per-write flag:
- **The in-memory `MemVfs` stays authoritative and synchronous** (unchanged hot path, still native-tested). Persistence is a background *write-behind*: the kernel serializes the durable subtree to an opaque byte blob (`Kernel::snapshot`), the **host** stores/loads that blob in IndexedDB, and boot **rehydrates** (`Kernel::hydrate`) before serving syscalls. The kernel never touches IndexedDB — Rust decides *what* is durable and serializes it; the host supplies the async storage mechanism it alone has (the same ADR-015/-020 "dumb byte mover" split).
- **Durability is path-based (a `MountTable`, `vfs/mount.rs`)** — the Unix `tmpfs`-at-`/tmp` model. Longest-prefix match; default policy persists root `/` and marks `/tmp` (user scratch) and the boot-reinstalled OS trees (`/bin`, `/sbin`, `/lib`) ephemeral. A per-inode durability flag was **rejected**: it forces every writer to decide, needs rewriting on `rename` across a boundary, and the ino-based `write_at` seam has no path to check. Path-based resolution at *snapshot* time makes cross-boundary `mv` correct for free and touches no inode state. An embedder can adjust the policy (`Kernel::mount(prefix, ephemeral)`); a per-file override could layer on later if ever needed, but v1 does not need it.
- **When to persist:** a monotonic `generation` counter bumps on every content/structure mutation; the host write-behind re-snapshots only when it advances past the last-stored value (an idle OS does zero I/O), on a ~2s debounce plus a `visibilitychange`/`pagehide` flush.
**Honest surface (INV-5):** durability is **eventually-consistent** — up to ~2s (or the last un-flushed change on a hard crash) can be lost; the hide/unload flush trims but cannot guarantee it. The snapshot is a **whole-durable-tree** blob in a single IndexedDB row (snapshots are small once `/tmp`+`node_modules` are pruned); per-file keying and delta writes are a future optimization if durable trees grow. No symlinks/perms/mtimes are persisted (the VFS has none). Quotas remain per-session until a durable quota home lands.
**Native-testable (INV-2):** the `MountTable` (longest-prefix, carve-outs), the snapshot/hydrate round-trip, ephemeral-subtree pruning, and the generation counter are all pure kernel bookkeeping, proven in `cargo test`; the IndexedDB write-behind + boot rehydration + reload-survival are proven in a headless-browser test (`tools/persistence.test.js`).
**Supersedes:** ADR-011's "IndexedDB `Vfs` behind the trait" phrasing. The trait boundary still stands; the *persistence mechanism* is a projection over it, not another implementation of it. COW snapshot/overlay ("reset project") remains future work and layers naturally on the same snapshot primitive.
**Revisit when:** the `Wasm`/Boa execution level (ADR-009) or a growing durable footprint motivates per-file keying / delta journaling in place of the whole-tree blob.
