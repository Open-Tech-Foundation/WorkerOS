...
//!
//! Scope: **relative** specifiers only (`./`, `../`, `/`), resolved against the
//! VFS. This is generic ES-module-on-a-filesystem resolution — no Node/npm
//! policy. Bare specifiers (`import _ from "lodash"`, `node:fs`) are **not** the
//! kernel's business (INV-1): `node_modules`/`package.json` `exports`/`node:`
//! builtins are Node-ecosystem convention that the guest node layer (`/bin/node`,
//! which has synchronous `fs`) resolves for itself. Here a bare specifier is an
//! explicit, honest error (the kernel graph is used to spawn OS programs, whose
//! imports are all relative/absolute).
//!
...
/// VFS path the kernel resolved it to (a relative specifier only — bare/`node:`
/// specifiers are the guest node layer's concern, not the kernel's).
...
pub fn resolve_graph(vfs: &dyn Vfs, entry: &str) -> Result<ModuleGraph, ResolveError> {
    ...
    while let Some(path_abs) = queue.pop() {
        ...
        for specifier in scan_imports(&source) {
            // Bare/`node:` specifiers are Node-ecosystem policy — the guest node
            // layer resolves those (INV-1). The kernel handles only relative ones.
            ...
            imports.push(ImportEdge { specifier, resolved });
        }
...
mod tests {
    ...
    fn bare_specifier_is_unsupported() {
        // Bare / `node:` specifiers are Node-ecosystem policy resolved by the
        // guest node layer, not the kernel (INV-1) — here they are an honest error.
        let mut vfs = MemVfs::new();
        ...
        write(&mut vfs, "/b.js", "import fs from 'node:fs';");
        assert_eq!(
            resolve_graph(&vfs, "/b.js").unwrap_err(),
            ResolveError::Unsupported("node:fs".into())
        );
    }
